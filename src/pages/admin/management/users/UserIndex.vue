...
import DialogChange from "pages/admin/management/users/Dialog/DialogChange.vue";
...
<template>
  <q-page padding>
    <DialogDelete v-if="dialog.delete && can('admin.management.users.deleteUser')"/>
    <DialogEdit v-if="dialog.edit && can('admin.management.users.updateUser')"/>
    <DialogChange v-if="dialog.password && can('admin.management.users.resetPassword')"/>
    <DialogCreate v-if="dialog.create && can('admin.management.users.createUser')"/>
    <q-card>
      <q-table
        ref="tableRef"
        ...
        :rows="table.data ?? []"
        :selection="can('admin.management.users.[deleteUser]') ? 'multiple' :'single'"
        binary-state-sort
        bordered
        row-key="id"
        @request="onRequest"
        ...
        <template v-slot:top>
          <q-toolbar class="text-primary">
            ...
            <div v-if="can('admin.management.users.[createUser,updateUser,deleteUser,resetPassword]')"
              ...
              <q-btn
                v-if="can('admin.management.users.updateUser')"
                :dense="$q.screen.lt.md"
              ...
              <q-btn
                v-if="can('admin.management.users.resetPassword')"
                :dense="$q.screen.lt.md"
              ...
              <q-btn
                v-if="can('admin.management.users.createUser')"
                :dense="$q.screen.lt.md"
